...
#define TRACE_LOCATION_FILE_LINE \
  hbann::ClampFileNameLength(TEXT(__FILE__), hbann::ToString(__LINE__), 50)
#define TRACE_LOCATION                                                                    \
  hbann::String(TRACE_LOCATION_PROCESS_THREAD_ID + TEXT(" ") + TRACE_LOCATION_FILE_LINE + \
                TEXT("\t"))
#define TRACE(strn) \
  hbann::Print<const ::TCHAR *>((TRACE_LOCATION + (hbann::StringStream() << strn).str()).c_str())
...
template <typename... Type>
constexpr bool is_basic_string_v<std::basic_string<Type...>> = true;
}  // namespace detail

template <typename Type>
constexpr bool is_basic_string_v = hbann::detail::is_basic_string_v<Type>;

template <typename Type>
using remove_decayed_cvptr_t = std::remove_const_t<std::remove_pointer_t<std::decay_t<Type>>>;
...
[[nodiscard]] static constexpr hbann::String ToString(const String & aString) noexcept
{
...
[[nodiscard]] static inline auto ClampFileNameLength(hbann::String         aFile,
                                                     hbann::String         aLine,
...
static constexpr void Print(
  const Range &                               aRange,
  const hbann::String &                       aSeparatorDimensions = TEXT("\n"),
  const std::function<void(const Object &)> & aFuncPrintElem =
    [](const auto & aObject)
  {
